...
- **Tests parte del alcance**: la suite de integración (508 tests) y la unit del front (325) quedan verdes en cada cambio. Migraciones EF: el SQL crudo (vistas) se mantiene a mano.
...
- **Backend** (`backend/`): `dotnet build AcgFotos.slnx` · `dotnet test AcgFotos.Api.IntegrationTests` (necesita PostgreSQL localhost; usa/crea `AcgFotos_Tests`) · host: `dotnet run --project AcgFotos.Api --launch-profile http` (→ :30000, env Development, DB `AcgFotos`).

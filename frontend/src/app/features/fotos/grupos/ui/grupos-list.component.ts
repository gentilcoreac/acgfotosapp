...
/** Valor del filtro que significa "todos los eventos" (GrupoCriteria.EventoId = 0). */
...
export class GruposListComponent {
  ...
  /** Se mergea a la query del listado (`GrupoCriteria.EventoId`); la tabla recarga al cambiar. */
  ...
  // Listado liviano (GrupoHeaderDto): los participantes se traen en el edit. `columns` es computed
  // porque la columna Evento depende del lookup (cuando carga, cambia la referencia del array y
  // la tabla la vuelve a tomar).
  protected readonly columns = computed<TbiColumn<Grupo>[]>(() => {
    ...
    return [
      ...
      { key: 'cantidadParticipantes', header: 'Participantes', align: 'center' },
    ];
  });

  protected readonly fetch = (query: QueryParams) => this.service.crud.getAllByCriteria(query);

  // La baja de un grupo con fotos la bloquea la API (el toast global muestra el motivo).
  protected readonly rowActions: TbiRowAction<Grupo>[] = [
    { icon: 'edit', label: 'Editar', handler: (row) => this.edit(row) },
    {
      ...
      confirm: (row) => ({
        title: 'Eliminar',
        message: `¿Eliminar el grupo "${row.nombre}" y sus participantes?`,
      }),

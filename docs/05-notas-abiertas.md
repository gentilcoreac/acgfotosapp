...
- [ ] **(Fase 1 — pedido del front al back)** Seed de menú del vertical Fotos: el front ya tiene las rutas `/fotos/eventos` y `/fotos/cursos` (ABMs admin), pero `allowedRoutesGuard` y el sidenav se alimentan de `gen_Menus` — hacen falta las filas de menú (con `routePath` `/fotos/eventos` y `/fotos/cursos`) en el seed de dev/tests para poder navegarlas sin darlas de alta a mano por el ABM de menús.

...
- [ ] **(Fase 1)** Upload masivo desde el navegador: DECIDIDO empezar por la API (permite generar derivados ahí mismo), con procesamiento en background (`Pendiente → Lista`, ya modelado) para no bloquear el request. Medir con un evento real; el escape si molesta es subida directa al bucket con URL firmada de escritura. **Regla de performance**: la API nunca proxya bytes de imagen hacia las familias en prod — solo firma URLs; el navegador baja del bucket/CDN.

...
- **UX de la pantalla de Fotos** (ideas 2026-07-14, priorizar con uso real):
  ...
  - ~~Ver una foto en pantalla completa y pasar a la anterior/siguiente sin salir (lightbox con flechas/teclado)~~ **HECHO (2026-08-10)**: `tbi-visor-fotos` (`shared/ui`) es el visor del sistema — aporta el marco de la foto, contador, cerrar, flechas y teclado, y delega en cada pantalla qué se dibuja y qué acciones ofrece. Lo adoptaron la galería admin, el detalle de pedidos, mi-álbum y el carrito; las dos primeras GANARON la navegación, que no tenían. Diseño a sangre completa con los datos sobre la foto (elegido comparando alternativas en la app real). **Cobertura ampliada a TODA imagen de la app** (pedido de Alberto 2026-08-11/12, no solo las 4 pantallas de arriba: QR de tarjetas, comparador de publicación, marca renderizada, miniatura de agregar-al-carrito, preview de subida) — **HECHO (2026-08-12)**: implementado y verificado a mano por Alberto contra el front/API dev reales. La primera verificación encontró 2 bugs reales que la suite unit no detectaba (diálogo de marca de agua en blanco por una condición de carrera real en `tbi-perfil-marca-agua-canvas` — el canvas dibujaba antes de que Angular le aplicara el tamaño vía binding, y redimensionarlo después borraba el dibujo por spec del elemento; y el comparador de `/fotos/publicacion`, que había migrado el carrusel inline pero nunca sumó el "click para ampliar" real) — corregidos y reverificados. Change archivado en `openspec/changes/archive/2026-08-12-visor-fotos-cobertura-total`.
